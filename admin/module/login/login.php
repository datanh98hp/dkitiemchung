<?php
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
    {
		...
		if(empty($error))
        {
			$sql = "SELECT * FROM admin WHERE ten_admin = '$username' AND matkhau_ad = '$password'";
			$result = $conn->query($sql);
			if ($result->num_rows == 1) {
				// output data of each row
				$row = $result->fetch_assoc() ;
				$_SESSION['admin_username'] = $row['ten_admin'];
				echo "<script>alert(' Đăng Nhập Thành Công');location.href='/dkitiemchung/admin'</script>";			
			  }
			  else{
